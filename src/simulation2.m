function mm = simulation2(deltamax)
  ...
  %% PARAMETERS OF THE CODE
    ...
    %% CONSTANTS
        ...
        % VELOCITY AND FORCE
            [w,w1,w2,T,F,freq,ciclo] = operation(Pow,W,db1,dp1,dp2); time = ciclo*cycle;
    ...
    %% DYNAMIC MODEL
        % SPACE-STATE SYSTEM
            [t1,t2,estados,estadostpm,acel,aceltpm,tvmf,tvmftpm] = model(step,cycle,time,Kteint,Ktmint,K1int,K2int,Cint,Cmint,d1int,d2int,F,me,memod,x0);

        % SIGNAL ANALYSIS
            % Steady-state
                % Displacement
                    [RMSx,RMSxtpm,Ampx,Ampxtpm,Espectrox,Espectroxtpm,ft,fttpm] = signal(estados,estadostpm,freq);
                    Fcx = Ampxtpm/RMSxtpm;
                ...
                % Force
                    [RMSf,RMSftpm,Ampf,Ampftpm,Espectrof,Espectroftpm,ft,fttpm] = signal(tvmf,tvmftpm,freq);

    %% PLOTS
         [mm] = plots(Ampx,Ampa,RMSx,RMSa,Ampxtpm,Ampatpm,RMSxtpm,RMSatpm,Kte,deltamax,w,theta1vg,C,vrelv,thetad,thetas,R1,Kteint,KexData,KeyData,Cint,CxData,CyData,t1,t2,estados,estadostpm,time,acel,aceltpm,tvmf,tvmftpm,Tauv,Ktem,TEexp,TEmexp,step,z1,delta,ft,fttpm);
