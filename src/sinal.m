function [RMS,RMStpm,Amp,Amptpm,Espectro,Espectrotpm,ft,fttpm] = sinal(estados,estadostpm,freq)
    ...
    Amp = peak2peak(abs(estados(size(estados(:,1),1)/2:size(estados(:,1),1),1))); % Amplitude pico do sistema original
    Amptpm = peak2peak(abs(estadostpm(size(estadostpm(:,1),1)/2:size(estadostpm(:,1),1),1))); % Amplitude pico do sistema modificado
