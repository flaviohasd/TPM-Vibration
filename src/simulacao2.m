  ...
  %% PARAMETROS UTILIZADOS NO CODIGO
    ...
    %% MODELO DINAMICO
        ...
        % ANALISE DO SINAL
            % Regime permanente
                % Deslocamento
                    [RMSx,RMSxtpm,Ampx,Ampxtpm,Espectrox,Espectroxtpm,ft,fttpm] = sinal(estados,estadostpm,freq);
                    Fcx = Ampxtpm/RMSxtpm;

                % Aceleracao
                    [RMSa,RMSatpm,Ampa,Ampatpm,Espectroa,Espectroatpm,ft,fttpm] = sinal(acel,aceltpm,freq);

                % Forca
                    [RMSf,RMSftpm,Ampf,Ampftpm,Espectrof,Espectroftpm,ft,fttpm] = sinal(tvmf,tvmftpm,freq);

    %% PLOTS
        [mm] = plots(Ampx,Ampa,RMSx,RMSa,Ampxtpm,Ampatpm,RMSxtpm,RMSatpm,Kte,deltamax,w,theta1vg,C,vrelv,thetad,thetas,R1,Kteint,KexData,KeyData,Cint,CxData,CyData,t1,t2,estados,estadostpm,tempo,acel,aceltpm,tvmf,tvmftpm,Tauv,Ktem,TEexp,TEmexp,step,z1,delta,ft,fttpm);
