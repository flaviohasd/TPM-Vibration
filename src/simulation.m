function RMSatpm = simulation(deltamax) % Perform the optimization using the RMS of the acceleration signal
  ...
  %% PARAMETERS OF THE CODE
        ...
        step = 1e4; % Quantity of iteractions to use for the stiffness calculation and numerical solution. (Use at least 1e4 to avoid big iteraction errors!)
%         deltamax = 43.5809; % Maximum amount of modification (µm)      %% Leave it commented for the optimization process.
    ...
    %% DYNAMIC MODEL
        % SPACE-STATE SYSTEM
            [t,states,statestpm,acel,aceltpm,dmf,dmftpm] = model(step,cycle,time,Kteint,Ktmint,K1int,K2int,Cint,Cmint,d1int,d2int,F,me,memod,x0,w);

        % SIGNAL ANALYSIS
            % Steady-state
                % Displacement
                    [RMSx,RMSxtpm,Ampx,Ampxtpm,Espectrox,Espectroxtpm,ftx,ftxtpm] = signal(states(:,1),statestpm(:,1),freq);

                % Velocity
                    [RMSv,RMSvtpm,Ampv,Ampvtpm,Espectrov,Espectrovtpm,ftv,ftvtpm] = signal(states(:,2),statestpm(:,2),freq);

                % Acceleration
                    [RMSa,RMSatpm,Ampa,Ampatpm,Espectroa,Espectroatpm,fta,ftatpm] = signal(acel,aceltpm,freq);

                % Force
                    [RMSf,RMSftpm,Ampf,Ampftpm,Espectrof,Espectroftpm,ftf,ftftpm] = signal(dmf,dmftpm,freq);

    %% PLOTS
%           [mm] = plots(Ampx,Ampa,RMSx,RMSa,Ampxtpm,Ampatpm,RMSxtpm,RMSatpm,Kte,deltamax,w,theta1vg,C,vrelv,thetad,thetas,R1,Kteint,KexData,KeyData,Cint,CxData,CyData,t,states,statestpm,time,acel,aceltpm,dmf,dmftpm,Tauv,Ktem,TEexp,TEmexp,step,z1,delta,ftx,ftxtpm);
