function [me,memod] = massa(z1,z2,p,deltamax,cr,pb,b,rb1,rb2,rf1,rf2,ri1,ri2,Sd1,Sd2,hr1,hr2,hi1,hi2,h1,h2,Sa1,Sa2)
    ...
    Ar1 = (deltamax/1000)*(hi1-hr1); % Area aproximada do material removido na modificacao do perfil dos dentes (mm^2)
    Ar2 = (deltamax/1000)*(hi2-hr2); % Area aproximada do material removido na modificacao do perfil dos dentes (mm^2)
    
    V1 = A1*b/(1e9); % Volume do dente 1 (m^3)
    V2 = A2*b/(1e9); % Volume do dente 2 (m^3)
    Vr1 = Ar1*b/(1e9); % Volume aproximado do material removido na modificacao do perfil dos dentes (m^3)
    Vr2 = Ar2*b/(1e9); % Volume aproximado do material removido na modificacao do perfil dos dentes (m^3)
    
    mD1 = V1*p; % Massa do dente 1 (kg)
    mD2 = V2*p; % Massa do dente 2 (kg)
    mDr1 = Vr1*p; % Massa aproximada que foi removida na modificacao do perfil dos dentes (kg)
    mDr2 = Vr2*p; % Massa aproximada que foi removida na modificacao do perfil dos dentes (kg)

    mDt1 = mD1*z1; % Massa de todos os dentes da engrenagem 1 (kg)
    mDt2 = mD2*z2; % Massa de todos os dentes da engrenagem 2 (kg)

    mDt1mod = (mD1-mDr1)*z1; % Massa de todos os dentes da engrenagem 1 modificada (kg)
    mDt2mod = (mD2-mDr2)*z2; % Massa de todos os dentes da engrenagem 2 modificada (kg)
    ...
    mt1 = mDt1 + m1c - m1cr; % Massa total da engrenagem 1 (kg)
    mt2 = mDt2 + m2c - m2cr; % Massa total da engrenagem 2 (kg)

    mt1mod = mDt1mod + m1c - m1cr; % Massa total da engrenagem 1 modificada (kg)
    mt2mod = mDt2mod + m2c - m2cr; % Massa total da engrenagem 2 modificada (kg)

    Id1 = mDt1*(ycg1/1000+ri1/1000)^2; % Momento de inercia dos dentes da engrenagem 1 (considerando toda massa concentrada na distancia do centro de gravidade)
    Id2 = mDt2*(ycg2/1000+ri2/1000)^2; % Momento de inercia dos dentes da engrenagem 2 (considerando toda massa concentrada na distancia do centro de gravidade)

    Id1mod = mDt1mod*(ycg1mod/1000+ri1/1000)^2; % Momento de inercia dos dentes da engrenagem 1 modificada (considerando toda massa concentrada na distancia do centro de gravidade)
    Id2mod = mDt2mod*(ycg2mod/1000+ri2/1000)^2; % Momento de inercia dos dentes da engrenagem 2 modificada (considerando toda massa concentrada na distancia do centro de gravidade)
    ...
    me = J1*J2/(J2*(rb1/1000)^2 + J1*(rb2/1000)^2); % Massa equivalente do sistema (kg)

    memod = J1mod*J2mod/(J2mod*(rb1/1000)^2 + J1mod*(rb2/1000)^2); % Massa equivalente do sistema modificado (kg)
