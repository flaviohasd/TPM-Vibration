function [mm] = plots(Ampx,Ampa,RMSx,RMSa,Ampxtpm,Ampatpm,RMSxtpm,RMSatpm,Kte,deltamax,w,theta1vg,C,vrelv,thetad,thetas,R1,Kteint,KexData,KeyData,Cint,CxData,CyData,t1,t2,estados,estadostpm,tempo,acel,aceltpm,tvmf,tvmftpm,Tauv,Ktem,TE,TEm,step,z1)
    
    % Frequencia
        sinal = fft(estados(:,1));
        f = (0:length(sinal)-1)*step/length(sinal); f = f(1:length(sinal)/2); sinal = abs(sinal(1:length(sinal)/2));

        sinaltpm = fft(estadostpm(:,1));
        ftpm = (0:length(sinaltpm)-1)*step/length(sinaltpm); ftpm = ftpm(1:length(sinaltpm)/2); sinaltpm = abs(sinaltpm(1:length(sinaltpm)/2));
    ...
    % RELACOES DE CONTATO
        ...
        xlabel('\theta_1 (°)')
        ylabel('v_{rel} (m/s)')
        xlim([0 round((2*thetad+thetas)*180/pi-1,1)])
    ...
    % ANALISE EM COORDENADA NORMALIZADA
        ...
        legend('TVMF','TVMF (mod)','Razão de Transmissão','Location','south','Orientation','horizontal')
        xlabel('\Gamma')
        ylabel('TVMF (N)')
        xlim([-0.5 0.5])
    ...
    % ANALISE DA FREQUENCIA
        ...
        plot(f,abs(sinal));hold on
        plot(ftpm,abs(sinaltpm))
        legend('\Delta_{máx} = 0 (\mum)',['\Delta_{máx} =',' ',num2str(deltamax),' ','(\mum)'],'Location','north','Orientation','horizontal')
        xlabel('f (Hz)')
        ylabel('Magnitude')
        xlim([0 200])   
